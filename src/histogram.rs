...
pub fn histogram_rgb(img: DynamicImage) -> Result<DynamicImage, anyhow::Error> {
    ...
    for y in 1..res.height() {
        ...
        for index in 0..vec_r.len() {
            ...
            new_val_g = max * (vec_g[index] as i32 - min_value_g as i32) / (g_max_g as i32 - min_value_g as i32);
            new_val_b = max * (vec_b[index] as i32 - min_value_b as i32) / (g_max_b as i32 - min_value_b as i32);

            // red
            color = if (new_val_r + 1) as i32 + y as i32 >= max as i32 { 0 } else { 255 };
            res.put_pixel((index as u32) * 4, y, Rgba([255, color, color, 1]) );

            // green
            color = if (new_val_g + 1) as i32 + y as i32 >= max as i32 { 0 } else { 255 };
            res.put_pixel((index as u32) * 4 + 1, y, Rgba([color, 255, color, 1]) );

            // blue
            color = if (new_val_b + 1) as i32 + y as i32 >= max as i32 { 0 } else { 255 };
